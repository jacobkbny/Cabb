...
func (state *State) Prepare(prepareMsg *VoteMsg) (*VoteMsg, error) {
	...
	if state == nil || state.CurrentStage != PrePrepared || !state.verifyMsg(prepareMsg.ViewID, prepareMsg.SequenceID, prepareMsg.Digest) {
...
func (state *State) Commit(commitMsg *VoteMsg) (*ReplyMsg, *RequestMsg, error) {
	...
	if state == nil || state.CurrentStage != Prepared || !state.verifyMsg(commitMsg.ViewID, commitMsg.SequenceID, commitMsg.Digest) {
	...
	if state.committed() {
		...
		return &ReplyMsg{
			...
			ClientID:  state.MsgLogs.ReqMsg.TxID,
			Result:    result,
